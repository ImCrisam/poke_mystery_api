...
constructor() {
  ...
  if (!apiKey) {
    throw new Error('❌ No se encontró API_TOKEN en las variables de entorno');
  }
  ...
  async createPokemonRiddleFromList(pokemons: any[]): Promise<any> {
    ...
    Instrucciones:
    - Escoge 4 índices de esta lista.
    ...
    {
      "options": [index1, index2, index3, index4],
      "answer": indexCorrecto,
    ...
    try {
      ...
      this.logger.debug(`Gemini raw response: ${JSON.stringify(response, null, 2)}`);
      ...
      this.logger.error(`Error en createPokemonRiddleFromList: ${error.message}`);
      throw error;
    }
  }
}
